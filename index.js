...
#bp-cssfont-box textarea { -webkit-user-select:auto; user-select:auto; }`;var css = document.createElement('style'); css.textContent = CSS_TEXT; document.head.appendChild(css);var dynStyle = document.createElement('style');
    ...
    <svg style="display:none;"><defs>
        ...
        <filter id="tex-fabric-filter"><feTurbulence type="fractalNoise" baseFrequency="0.30.05" numOctaves="3" stitchTiles="stitch"/></filter>
        <filter id="tex-scratch-filter"><feTurbulence type="turbulence" baseFrequency="0.010.4" numOctaves="2" stitchTiles="stitch"/></filter>
    </defs></svg>
    <div id="top-left-bar">
        ...
        <div class="icon-btn" onclick="toggleLayoutPanel()" title="布局"><svg viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="7.5" rx="1.5" stroke="#1C1C1C" stroke-width="1.6"/><rect x="3" y="13.5" width="18" height="7.5" rx="1.5" stroke="#1C1C1C" stroke-width="1.6"/></svg></div>
        <div class="icon-btn" onclick="toggleShapePanel()" title="抠字形状"><svg viewBox="0 0 24 24" fill="none" stroke="#1C1C1C" stroke-width="1.6"><polygon points="12,2 15,9 22,9 16.5,13.5 18.5,21 12,16.5 5.5,21 7.5,13.5 2,9 9,9" fill="none"/></svg></div>
    </div>
    ...
    //===== 可拖动悬浮按钮 =====
    var bubbleBtn = document.createElement('div'); bubbleBtn.id='bp-bubble-btn'; bubbleBtn.innerHTML='&#x1FAE7;';
    ...
    (function(){
        var bDrag=false,bSX,bSY,bOX,bOY,bMoved=false;
        ...
        window.addEventListener('touchmove',function(e){if(!bDrag)return;bMoved=true;var p=e.touches[0];var nx=bOX+(p.clientX-bSX),ny=bOY+(p.clientY-bSY);bubbleBtn.style.right='auto';bubbleBtn.style.transform='none';bubbleBtn.style.left=Math.max(0,Math.min(window.innerWidth-40,nx))+'px';bubbleBtn.style.top=Math.max(0,Math.min(window.innerHeight-40,ny))+'px';},{passive:true});
        ...
        window.addEventListener('mousemove',function(e){if(!bDrag)return;bMoved=true;var nx=bOX+(e.clientX-bSX),ny=bOY+(e.clientY-bSY);bubbleBtn.style.right='auto';bubbleBtn.style.transform='none';bubbleBtn.style.left=Math.max(0,Math.min(window.innerWidth-40,nx))+'px';bubbleBtn.style.top=Math.max(0,Math.min(window.innerHeight-40,ny))+'px';});
    ...
    window.openBpApp=function(text){container.style.display='flex';renderArticle(text||defaultText);setTimeout(function(){syncCollageSize();updateBottomMeta();if(!text){collageArea.style.height='160px';var ws='我爱你'.split('');var rect=collageArea.getBoundingClientRect();var aW=rect.width||400;var sX=(aW-ws.length*36)/2;if(sX<15)sX=15;ws.forEach(function(ch,i){var sc=document.createElement('div');sc.className='scrap-word bp-welcome-scrap';sc.textContent=ch;sc.style.left=(sX+i*36)+'px';sc.style.top='60px';sc.style.opacity='0.7';bindDrag(sc);collageArea.appendChild(sc);});}},100);};
    ...
    // ===== 字块大小 =====
    function setScrapSize(w,h){root.style.setProperty('--scrap-w',w+'px');root.style.setProperty('--scrap-h',h+'px');scrapSize=Math.max(w,h);}

    // ===== 布局面板 =====
    function renderLayoutPanel(){var p=document.getElementById('layout-panel');p.innerHTML='<div style="font-size:9px;color:#888;margin-bottom:6px;font-weight:600;letter-spacing:0.1em;">布局方式</div><div class="layout-grid"><button class="action-chip '+(currentLayout==='vertical'?'active':'')+'" onclick="switchLayout(\'vertical\')">上下</button><button class="action-chip '+(currentLayout==='horizontal'?'active':'')+'" onclick="switchLayout(\'horizontal\')">左右</button><button class="action-chip" onclick="swapAreas();toggleLayoutPanel();">交换</button></div>';}
    ...
    function renderShapePanel(){
        ...
        var curW=parseInt(root.style.getPropertyValue('--scrap-w'))||26;
        html+='<div class="slider-row"><span>字块大小</span><input type="range" min="18" max="44" value="'+curW+'" id="scrap-size-slider" oninput="onScrapSizeChange(this.value)"><span class="slider-val" id="scrap-size-val">'+curW+'px</span></div>';
        ...
        panel.querySelectorAll('.shape-item').forEach(function(el){
            el.addEventListener('click',function(){
                ...
                if(currentCutShape==='default'){setScrapSize(26,30);}
                else if(currentCutShape==='square'){setScrapSize(28,28);}
                applyShapeToAll();renderShapePanel();
            });
        });
    ...
    function onScrapSizeChange(v){
        v=parseInt(v);
        if(currentCutShape==='default'){setScrapSize(v,Math.round(v*30/26));}
        else{setScrapSize(v,v);}
        var valEl=document.getElementById('scrap-size-val');if(valEl)valEl.textContent=v+'px';
    }
    ...
    // ===== 形状应用 =====
    function applyShapeToCut(el){el.classList.remove('shape-symbol','shape-image');el.removeAttribute('data-symbol');if(currentCutShape==='symbol'){el.classList.add('shape-symbol');el.setAttribute('data-symbol',customSymbol||'x');}else if(currentCutShape==='image'&&customShapeDataUrl){el.classList.add('shape-image');el.style.setProperty('--cut-mask','url('+customShapeDataUrl+')');}}
    function applyShapeToScrap(el){el.style.removeProperty('clip-path');el.style.removeProperty('-webkit-clip-path');if(!syncShapeToScrap)return;if(currentCutShape==='symbol'||currentCutShape==='image'||currentCutShape==='default')return;var clip=getClipForShape(currentCutShape);if(clip){el.style.clipPath=clip;el.style.webkitClipPath=clip;}}
    function applyShapeToAll(){document.querySelectorAll('#bp-app-container .char-node.is-cut').forEach(applyShapeToCut);document.querySelectorAll('#bp-app-container .scrap-word').forEach(applyShapeToScrap);var clip=getClipForShape(currentCutShape);if(currentCutShape!=='symbol'&&currentCutShape!=='image'&&clip){root.style.setProperty('--cut-clip',clip);}else{root.style.removeProperty('--cut-clip');}}
    ...
    function resetCuts(){textFlow.querySelectorAll('.char-node.is-cut').forEach(function(n){n.classList.remove('is-cut','shape-symbol','shape-image');n.style.removeProperty('--cut-mask');});collageArea.querySelectorAll('.scrap-word').forEach(function(n){n.remove();});}
