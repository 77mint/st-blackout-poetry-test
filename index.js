...
jQuery(async () => {
    ...
    function saveCustomFontsToStorage(){ openBpDB().then(function(db){ var tx=db.transaction(BP_DB_STORE,'readwrite'); tx.objectStore(BP_DB_STORE).put(customFonts,'customFonts'); }).catch(function(e){console.warn('[拼贴诗] 字体保存失败',e);}); }
    function loadCustomFontsFromStorage(){ return openBpDB().then(function(db){ return new Promise(function(resolve){ var tx=db.transaction(BP_DB_STORE,'readonly'); var req=tx.objectStore(BP_DB_STORE).get('customFonts'); req.onsuccess=function(e){ if(Array.isArray(e.target.result))customFonts=e.target.result; resolve(); }; req.onerror=function(){resolve();}; }); }).catch(function(e){console.warn('[拼贴诗] 字体读取失败',e);}); }
...
#bp-cssfont-box textarea { -webkit-user-select:auto; user-select:auto; }
    ...
    window.openBpApp=function(text){container.style.display='flex';renderArticle(text||defaultText);setTimeout(function(){syncCollageSize();updateBottomMeta();if(!text){collageArea.style.height='200px';var ws='欢迎使用薄荷的拼贴诗'.split(''),rect=collageArea.getBoundingClientRect(),aW=rect.width||400,sX=(aW-ws.length*34)/2;if(sX<15)sX=15;ws.forEach(function(ch,i){var sc=document.createElement('div');sc.className='scrap-word bp-welcome-scrap';sc.textContent=ch;sc.style.left=(sX+i*34)+'px';sc.style.top=(80+parseInt((Math.random()*24-12).toFixed(0)))+'px';sc.style.transform='rotate('+(Math.random()*16-8).toFixed(1)+'deg)';sc.style.opacity='0.55';bindDrag(sc);collageArea.appendChild(sc);});}},100);};
